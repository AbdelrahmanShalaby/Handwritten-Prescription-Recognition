...
# import sys
# sys.path.append(r'C:\Users\Joe\Anaconda3\envs\trail\Lib\site-packages')
...
Whole_String_Of_Classes=" \"!#&'()*+,-./0123456789:;?ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"

f = open("D:/Important Files/Graduation Project/Proto/samples/Testing_List", "r")

res = f.read()

lines = res.split("\n")

cfg.TEST_NB = len(lines)
cfg.TEST_NB-=1
print(cfg.TEST_NB)
# Reading test data...
InputListTest, SeqLensTest, _ = ReadData(cfg.TEST_LOCATION, cfg.TEST_LIST,
                                         cfg.TEST_NB, WND_HEIGHT, WND_WIDTH, WND_SHIFT, VEC_PER_WND, '')
        ...
        for i in range(0, cfg.BatchSize):
            ...
            if cfg.WriteDecodedToFile == True:
                ...
                for i in range(len(decodedStr)):
                    if decodedStr[i] in Whole_String_Of_Classes:
                        temp+=decodedStr[i]
                DecodeLog.write(temp+'\n')
            else:
