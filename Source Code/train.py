...
inputList, seqLens, targetList = ReadData(cfg.TRAIN_LOCATION, cfg.TRAIN_LIST, cfg.TRAIN_NB, WND_HEIGHT, WND_WIDTH,
                                          WND_SHIFT, VEC_PER_WND, cfg.TRAIN_TRANS)

# Reading validation data...
if (cfg.VAL_NB > 0): inputListVal, seqLensVal, targetListVal = ReadData(cfg.VAL_LOCATION, cfg.VAL_LIST, cfg.VAL_NB,
                                                                        WND_HEIGHT, WND_WIDTH, WND_SHIFT, VEC_PER_WND,
                                                                        cfg.VAL_TRANS)
...
try:
  for epoch in range(cfg.StartingEpoch, cfg.NEpochs):
    ...
    while end <= len(inputList):
      ...
      feed = {x: batchInputs, SeqLens: batchSeqLengths, indices: batchTargetIxs, values: batchTargetVals,
              shape: batchTargetShape}
      del batchInputs, batchTargetIxs, batchTargetVals, batchTargetShape, batchSeqLengths

      _, summary, Losses, Loss, Error = session.run([train_step, LocalTrainSummary, losses, loss, error_rate],
                                                    feed_dict=feed)
      del feed
