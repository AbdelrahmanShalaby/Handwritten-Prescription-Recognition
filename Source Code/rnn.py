...
def RNN(Inputs, SeqLens, Scope):
	...
	with tf.variable_scope(Scope):
		...
		i, Inputs = tf.while_loop(condition, body, [i, Inputs], shape_invariants=[i.get_shape(),
																				  tf.TensorShape([None, cfg.BatchSize, NFeatures])])
		...
		for i in range(cfg.NLayers):
			stacked_rnn_forward.append(tf.nn.rnn_cell.LSTMCell(num_units=cfg.NUnits, initializer=initializer,
		...
		for i in range(cfg.NLayers):
			stacked_rnn_backward.append(tf.nn.rnn_cell.LSTMCell(num_units=cfg.NUnits, initializer=initializer,
																use_peepholes=True, state_is_tuple=True))
		backward = tf.nn.rnn_cell.MultiRNNCell(stacked_rnn_backward, state_is_tuple=True)

		[fw_out, bw_out], _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=forward, cell_bw=backward, inputs=Inputs, time_major=True,
															  dtype=tf.float32,sequence_length=tf.cast(SeqLens, tf.int64))
		...
		# Linear Layer params
		W_fw = tf.Variable(tf.truncated_normal(shape=[cfg.NUnits, NClasses],
											   stddev=np.sqrt(2.0 / cfg.NUnits), dtype=tf.float32), dtype=tf.float32)
		W_bw = tf.Variable(tf.truncated_normal(shape=[cfg.NUnits, NClasses],
											   stddev=np.sqrt(2.0 / cfg.NUnits), dtype=tf.float32), dtype=tf.float32)
